...
app.get('/transactions', function(request, response, next) {
  ...
  var startDate = moment().subtract(100, 'days').format('YYYY-MM-DD');
  ...
  client.getTransactions(ACCESS_TOKEN, startDate, endDate, {
    ...
    if (error != null) {
      ...
      //custom code
      const accountIdVisa = transactionsResponse.accounts.find(({name}) => name.includes("Alaska")).account_id;
      const accountIdMastercard = transactionsResponse.accounts.find(({name}) => name === "Loan Mastercard").account_id;
      const filtered = transactionsResponse.transactions
          .filter(({account_id})=>account_id === accountIdVisa || account_id === accountIdMastercard)
      ...
      fs.writeFileSync(`downloads/transactions_${startDate}_${endDate}.csv`, csv);
      //
